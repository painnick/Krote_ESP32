...
template<class T_SERIAL_METHOD, class T_NOTIFICATION_METHOD, class T_CHIP_VARIANT = Mp3ChipMH2024K16SS>
#else
            ...
            if (readPacket(&replyCommand, &replyArg)) {
                if (command != 0 && command == replyCommand) {
                    ...
                    switch (replyCommand) {
                        ...
                        case 0x3A:
                            _isOnline = true;
                            T_NOTIFICATION_METHOD::OnPlaySourceInserted(*this,
                                                                        static_cast<DfMp3_PlaySources>(replyArg));
                            break;
