...
bool validateChecksum(const DfMp3_Packet_WithCheckSum &in) {
  ...
  return (sum ==
          static_cast<uint16_t>((in.hiByteCheckSum << 8) | in.lowByteCheckSum));
}
...
public:
  ...
  void loop() {
    while (_serial.available() >=
           static_cast<int>(sizeof(typename T_CHIP_VARIANT::ReceptionPacket))) {
      listenForReply(0x00);
    }
  }
...
private:
  ...
  void sendPacket(uint8_t command, uint16_t arg = 0,
    ...
    typename T_CHIP_VARIANT::SendPacket packet =
        T_CHIP_VARIANT::generatePacket(command, arg);
      ...
      if (readPacket(&replyCommand, &replyArg)) {
        if (command != 0 && command == replyCommand) {
          ...
          case 0x4b: // usb on MH2024K-16SS
            T_NOTIFICATION_METHOD::OnPlayFinished(*this, DfMp3_PlaySources_Usb,
                                                  replyArg);
            break;

          case 0x3d: // micro sd
          case 0x4c: // micro sd on MH2024K-16SS
            T_NOTIFICATION_METHOD::OnPlayFinished(*this, DfMp3_PlaySources_Sd,
                                                  replyArg);
            break;
